        ...
        if (subnet):
            ...
            if services[service] == True:
                if service_port_map is None:
                    raise ValueError("Critical Error: 'service_port_map' is None. The configuration file or tiny.yml failed to load the service-to-port mappings.")
                port = service_port_map[service]
                port_list.append(port)
...
def clean_alertList():
    """
    Clear the alerts generated by the IDS, cleaning the environment for a new episode
    """
    global alerts
    alerts.clear()
    return True
...
def block_connections(target_ip=None, rule_path="/etc/snort/rules/local.rules"):
    """
    Synthesizes dynamic packet filtering heuristics within the NIDS by parsing the 
    global alerts tensor. Forces the Snort DAQ engine to actively drop datagrams 
    associated with anomalous flows.
    """
    global alerts,current_sid
    ...
    mitigated_vectors = set()

    for alert in alerts:
        # Extract flow metadata from the parsed telemetry dictionary
        src_ip = alert.get("src_ip")
        dst_ip = alert.get("dst_ip")
        
        # If a specific target node is specified by the orchestration logic, filter extraneous telemetry
        if target_ip and src_ip != target_ip and dst_ip != target_ip:
            continue
            
        # Standardize the protocol string for Snort's rule parser
        protocol = alert.get("protocol", "ip").lower()
        
        flow_signature = (src_ip, dst_ip, protocol)
        
        if flow_signature not in mitigated_vectors:
            # Synthesize a unidirectional drop rule strictly mapped to the observed adversarial vector
            rule = f'drop {protocol} {src_ip} any -> {dst_ip} any (msg:"BlueTeam - Contextual Mitigation {src_ip} to {dst_ip}"; sid:{current_sid}; rev:1;)'
            mitigation_rules.append(rule)
            
            # Register the vector to prevent redundant processing
            mitigated_vectors.add(flow_signature)
            current_sid += 1
...
def unlock_connections(backup_path="/etc/snort/rules/local.rules.bak", rule_path="/etc/snort/rules/local.rules"):
    """
    Executes a deterministic policy rollback within the NIDS engine to deprecate 
